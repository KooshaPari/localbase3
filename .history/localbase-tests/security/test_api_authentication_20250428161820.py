...
def test_api_key_authentication(setup):
    ...
    assert response.status_code in [
        401,
        403,
    ], f"API accessible without API key: {response.status_code}"
    ...
    assert response.status_code in [
        401,
        403,
    ], f"API accessible with invalid API key: {response.status_code}"
    ...
    ), f"API not accessible with valid API key: {response.status_code}"
...
def test_cors_headers(setup):
    ...
    assert (
        len([h for h in response.headers if h.startswith("Access-Control-")]) > 0
...
def test_openai_api_compatibility(setup):
    ...
    assert (
        response.status_code == 200
    ), f"OpenAI models endpoint not accessible: {response.status_code}"
    ...
    assert (
        response.status_code == 200
    ), f"OpenAI chat completions endpoint not accessible: {response.status_code}"
