...
def test_provider_update(setup):
    ...
    response = (
        supabase.table("providers").update(updated_data).eq("id", provider_id).execute()
    )
    ...
    assert (
        response.data[0]["description"] == updated_data["description"]
    ), "Provider description not updated"
    assert (
        response.data[0]["models"] == updated_data["models"]
    ), "Provider models not updated"
