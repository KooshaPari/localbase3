...
def test_signout(setup):
    ...
    assert "next-auth.session-token" not in frontend.cookies, "Session cookie not removed"
...
def test_protected_route(setup):
    ...
    assert response.status_code in [401, 403], f"Protected route accessible without authentication: {response.status_code}"
    ...
    # Should succeed
    assert response.status_code == 200, f"Protected route not accessible with authentication: {response.status_code}"
