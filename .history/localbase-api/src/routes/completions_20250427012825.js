 ...
 */
function mockCompletion(request) {
  return new Promise(resolve => {
    setTimeout(() => {
      ...
      if (request.prompt.toLowerCase().includes('hello') || 
        ...
        responseText = 'Hello! This is a sample response from the LocalBase API.';
      } else if (request.prompt.toLowerCase().includes('help')) {
        responseText = 'I can help you with various tasks. This is a sample response.';
...
router.post('/', authenticate, async (req, res, next) => {
  try {
    ...
    if (req.body.prompt === undefined) {
      throw new InvalidRequestError('prompt is required', 'prompt', 'param_required');
    }
