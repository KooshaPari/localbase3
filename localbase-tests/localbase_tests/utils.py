...
def load_config() -> Dict[str, Any]:
    ...
    if not os.path.exists(config_path):
        ...
        config = {
            ...
            "provider": {"url": "http://localhost:8000", "api_key": "lb_test_key"},
        }
...
def get_supabase_client():
    ...
    return SupabaseMockClient(
        supabase_config.get("url"), supabase_config.get("anon_key")
    )
...
def get_blockchain_client() -> Tuple[Any, Any]:
    ...
    return (
        BlockchainMockClient(
            blockchain_config.get("url"), blockchain_config.get("chain_id")
        ),
...
def get_supabase_admin_client():
    ...
    # Return a mock client for now
    class SupabaseAdminMockClient:
        def __init__(self, url, key):
            self.url = url
            self.key = key

    return SupabaseAdminMockClient(
        supabase_config.get("url"), supabase_config.get("anon_key")
    )


def create_test_job(
    provider_url: str, api_key: str, model: str, prompt: str
) -> Dict[str, Any]:
    """
...
def wait_for_job_completion(
    provider_url: str, api_key: str, job_id: str, timeout: int = 60
) -> Dict[str, Any]:
    """
